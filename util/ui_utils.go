...
func UpdateFocusBorders(focusOrder []tview.Primitive, currentFocusIndex int, activeBorderColor tcell.Color) {
	for i, view := range focusOrder {
		// Check if the view has border-related methods (i.e., if it's a tview.Box)
		if box, ok := view.(*tview.Box); ok {
			// If this is the currently focused view, update its border
			if i == currentFocusIndex {
				box.SetBorder(true)
				box.SetBorderColor(activeBorderColor)
				box.SetBorderPadding(1, 1, 1, 1) // Optional: Adjust padding as needed
			} else {
				// Reset the border for other views
				box.SetBorder(false)
			}
		}
	}
